...
# --- 設定読み込み ---
# .env ファイルを読み込み、そこに記述された変数を環境変数として使えるようにする
load_dotenv()
# 環境変数からBotトークンを取得 (例: STUDY_REMIND_BOT_TOKEN)
TOKEN = os.getenv('DISCORD_BOT_TOKEN')
# 環境変数からチャンネルIDを取得 (例: STUDY_REMIND_CHANNEL_ID)
CHANNEL_ID_STR = os.getenv('REMINDER_CHANNEL_ID')
...
if CHANNEL_ID_STR:
    try:
        ...
        # 文字列が数値に変換できない場合はエラーメッセージを表示
        print(f"設定エラー: .envファイル内のチャンネルID '{CHANNEL_ID_STR}' は有効な数値ではありません。")
        # ここでプログラムを終了させるか、Noneのまま続行するかは設計による
        # exit() # 終了させる場合
else:
    # チャンネルID自体が.envファイルに設定されていない場合
    print("設定エラー: .envファイルにチャンネルID (STUDY_REMIND_CHANNEL_ID) が設定されていません。")
    # exit() # 終了させる場合

# --- 発表者リストの読み込み (.env から JSON形式で) ---
PRESENTERS = [] # デフォルトは空リスト
# ↓ .env ファイルに定義したキー名 (輪読用) に合わせる
presenter_json_string = os.getenv('PRESENTERS_JOURNAL')
            ...
            for p in loaded_data:
                if 'name' in p and 'id' in p:
                    try:
                        ...
                        print(f"設定警告(Journal): .envのJSON内のID '{p.get('id')}' は無効な整数。スキップ。")
                        all_valid = False
                else:
                    print(f"設定警告(Journal): .envのJSON内の要素 {p} に 'name'/'id' 不足。スキップ。")
                    all_valid = False

            if all_valid and valid_presenters:
                PRESENTERS = valid_presenters
                print(f"DEBUG(Journal): .envから読み込んだ発表者リスト: {PRESENTERS}")
            elif valid_presenters:
                PRESENTERS = valid_presenters
                print(f"DEBUG(Journal): .envから一部無効なデータを除き読み込んだ発表者リスト: {PRESENTERS}")
            else:
                print("設定エラー(Journal): .envのJSONから有効な発表者データを読み込めませんでした。")

        else:
            print("設定エラー(Journal): .envの JSON 形式が不正（辞書のリストではない）。")

    except json.JSONDecodeError as e:
        print(f"設定エラー(Journal): .envの JSON 形式が不正。エラー: {e}")

else:
    # 環境変数自体が設定されていない場合
    print("警告(Journal): .env ファイルに 'JOURNAL_PRESENTER_LIST_JSON' 未設定。")

# 発表者リストが最終的に空でないかチェック
if not PRESENTERS:
    print("致命的エラー(Journal): 利用可能な発表者リストがありません。Botを実行できません。")
    exit("エラー(Journal): 発表者リストが空のため終了します。") # 終了させる
...
# schedule.every().thursday.at("15:10").do(reminder_job)
# schedule.every().thursday.at("16:50").do(next_presenter_notification_job)
# print("本番用スケジュール設定: 木曜 15:10 (通常), 16:50 (次回発表者)")
