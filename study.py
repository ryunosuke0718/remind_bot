...
if __name__ == "__main__":
    ...
    # 起動前に設定値を確認 (study_Coreから読み込む)
    if study_Core.TOKEN is None: # 参照元を変更
        print("起動エラー: Botトークンが設定されていません。 .env と study_Core.py を確認してください。")
    elif study_Core.CHANNEL_ID is None: # 参照元を変更
        print("起動エラー: チャンネルIDが設定されていないか無効です。 .env と study_Core.py を確認してください。")
    # 発表者リストが空でも起動は試みる（警告はon_readyで出す）
    else:
        try:
            print("Discordに接続し、Botを起動します...")
            # study_Core で定義・準備された client と TOKEN を使って Bot を実行
            study_Core.client.run(study_Core.TOKEN) # 参照元を変更
